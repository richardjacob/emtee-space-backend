                    ...
                    <div class="col-12 p-0 my-3">
                        ...
                        <div class="listing cls_dis_rimg">
                            <div class="panel-image listing-img img-large">
                                <a href="{{ url('space/'.$dispute->reservation->space_id) }}" class="media-photo media-cover wishlist-bg-img" target="_blank">
                                    <img src="{{ $dispute->reservation->space->photo_name }}" width="639" height="426">
                                </a>
                            </div>
                            <div class="panel-body panel-card-section">
                                <div class="media">
                                    <a href="{{ url('users/show/'.$dispute->reservation->space->user_id) }}" target="_blank" class="pull-right media-photo media-round card-profile-picture card-profile-picture-offset" title="{{ $dispute->reservation->space->users->first_name }}">
                                        <img src="{{ $dispute->reservation->space->users->profile_picture->src }}" height="60" width="60" alt="{{ $dispute->reservation->space->users->first_name }}">
                                    </a>
                                    <div class="py-4" style="width: 100%">
                                        <a href="{{ url('space/'.$dispute->reservation->space->id) }}" class="text-normal" target="_blank">
                                            <div title="{{ $dispute->reservation->space->name }}" class="h5 listing-name text-truncate row-space-top-1">
                                                {{ $dispute->reservation->space->name }}
                                            </div>
                                        </a>
                                        <div class="">
                                            <span>{{$dispute->reservation->created_at_date}}</span>
                                            <span class="dot-cont">·</span>
                                            <span>{{$dispute->reservation->hours}} {{trans_choice('messages.booking.hour', $dispute->reservation->hours)}}</span>
                                            <span class="dot-cont">·</span>
                    ...
                    <div class="col-12 p-0 my-3">
                        ...
                        <div class="dispute-gallery">
                            <ul>
                                ...
                                <li class="cls_disphoto1 mb-3">
                                    <div data-thumb="{{ $dispute_doc->file_url }}" data-src="{{ $dispute_doc->file_url }}" data-sub-html=".caption_{{ $dispute_doc->file_url }}" class="cls_disphoto">
