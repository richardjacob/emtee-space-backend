...
@section('inbox_address')
  ...
  <span ng-if="all.user_to != all.user_from">
    <span class="street-address" ng-if="all.space.space_address.address_line_1 || all.space.space_address.address_line_2">
      @{{ all.space.space_address.address_line_1 }} @{{ all.space.space_address.address_line_2 }},
    </span>
    <span class="locality" ng-if= "all.space.space_address.city">
      @{{ all.space.space_address.city }},
    </span>
    <span class="region">
      @{{ all.space.space_address.state }}
    </span>
    <span ng-hide="all.reservation.type == 'contact'">
    ...
    <div id="inbox" class="inbox-wrap mt-4" ng-cloak>
      ...
      <ul class="inbox-list border-top-0">
        <li class="d-flex py-3 py-md-4" ng-repeat="all in message_result.data" ng-class="(all.read == '1') ? 'readed' : ''" ng-cloak>
          ...
          <!-- Profile Picture -->
          <div class="col-9 col-md-10 p-0 d-md-flex mt-2 mt-lg-3">
            <!-- Name, Date And Time -->
            <div class="list-name pl-md-0 col-12 col-md-3">
            ...
            <div class="reserve-link col-12 col-md-4" ng-if="all.user_from == all.user_to">
              ...
              <a ng-if="all.space_id != 0" href="{{ url('messaging/admin')}}/@{{ all.reservation_id }}">
                @yield('inbox_address')
              </a>
              <!-- Admin Verification Messages -->
              <a ng-if="all.space_id == 0" href="{{ url('admin_messages')}}/@{{ all.user_to }}">
                @yield('inbox_address')
              </a>
            </div>
            <!-- Conversation and Address Details -->
