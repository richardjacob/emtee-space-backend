            ...
            <div class="card-body photos-section" ng-init="original_src='{{ $result->profile_picture->getOriginal('src') }}'">
              <div class="row">
                ...
                <div class="col-lg-8 mt-4 mt-lg-0 text-center text-md-left">
                  ...
                  <div class="btn file-input-container webcam_photo d-none" data-toggle="modal" data-target="#takephoto-popup"> {{ trans('messages.profile.take_photo') }}</div>
                </div>
              </div>
            </div>
          </div>
...
function take_snapshot() {
  ...
  $('#takephoto_button').show();
  document.getElementById('takephoto_result').innerHTML = 
  '<img id="imageprev" src="'+data_uri+'"/>';
    $('#user_takephoto_pic').val(data_uri);
  });
}


function clear_takepic() {
  $('#takephoto_button').hide();
  $('#imageprev').attr('src','');
}
