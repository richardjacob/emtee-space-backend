    ...
    <div class="review-wrap d-md-flex row py-4 py-md-5">
      <div class="review-user-info col-md-4">
        ...
        <div class="panel-image">
          <a title="{{ $result->space->name }}" class="media-photo media-cover text-center" alt="{{ $result->space->name }}" href="{{ $result->space->link }}">
            <img src="{{ $result->space->photo_name }}">
          </a>  
        </div>
        <div class="panel-info">
          <a class="profile-image" href="{{ route('show_profile',[$result->space->users->id]) }}">
            <img title="{{ $result->space->users->first_name }}" src="{{ $result->space->users->profile_picture->src }}" alt="{{ $result->space->users->first_name }}">
          </a>
          <h4 class="text-truncate">
            {{ $result->space->name }}
          </h4>
          <span>
            {{ $result->space->room_type_name }} · {{ $result->space->space_address->city }}
          </span>
          <h4 class="text-truncate">
            {{ trans('messages.reviews.hosted_by') }}
            {{ $result->space->users->full_name }}
          </h4>
          <span>
            {{ $result->dates_subject }}
          </span>
        </div>
      </div>
          ...
          <div class="review-facets panel-body">
            <div id="double-blind-copy" class="review-facet">
              <section class="mt-3 mt-md-5">
                <h3>
                  {{ trans('messages.reviews.write_review_for') }} {{ $result->space->users->first_name }}
                </h3>
                <p>
