        ...
        <div class="col-12 col-md-9">
          <div class="card">
            <div class="card-header d-md-flex text-center justify-content-between">
              <h6 class="m-0">
                @lang('messages.your_reservations.requested_reservation')
              </h6>
              ...
              <div class="reserve-left mt-2 mt-md-0">
                <span class="label label-info green-color d-inline-flex align-items-center">
                  ...
                  @lang('messages.your_reservations.expires_in')
            ...
            <div id="accept_decline" class="card-body">
              <div class="table-responsive">
                <table class="table table-bordered">
                  <tbody>
                    <tr>
                      <td>@lang('messages.your_reservations.property')</td>
                      <td>
                        {{ $result->space->name }}
                        <a class="theme-link table-link" href="{{ $result->space->link }}" data-popup="true">
                          @lang('messages.your_reservations.view_property')
                        </a>
                      </td>
                    </tr>
                    <tr>
                      <td>@lang('messages.space_detail.event_type')</td>
                      <td>
                        {{ $result->event_type_name }}
                      </td>
                    </tr>
                    @if($result->booking_period == 'Multiple')
                    <tr>
                      <td>@lang('messages.your_reservations.checkin')</td>
                      <td>
                        {{ $result->reservation_times->checkin_formatted }}
                        - {{ $result->reservation_times->start_time_formatted }}
                      </td>
                    </tr>
                    <tr>
                      <td>@lang('messages.your_reservations.checkout')</td>
                      <td>
                        {{ $result->reservation_times->checkout_formatted }}
                        - {{ $result->reservation_times->end_time_formatted }}
                        <a class="theme-link table-link" href="{{ route('manage_space',['space_id' => $result->space_id, 'page' => 'ready_to_host', 'step_num' => 5]) }}">
                          @lang('messages.your_reservations.view_calendar')
                        </a>
                      </td>
                    </tr>
                    @else
                      <tr>
                        ...
                        <td>
                          {{ $result->reservation_times->checkin_formatted }}
                          ({{ $result->reservation_times->times_formatted }})
                          <a class="theme-link table-link" href="{{ route('manage_space',['space_id' => $result->space_id, 'page' => 'ready_to_host', 'step_num' => 5]) }}">
                            @lang('messages.your_reservations.view_calendar')
                          </a>
                        </td>
                    ...
                    {{--<tr>
                      <td>
                        @choice('messages.booking.hour',2)
                      </td>
                      <td>
                        {{ $result->hours }}
                      </td>
                    ...
                    <tr>
                      <td> @lang('messages.new_space.maximum_guests')</td>
                      <td>
                    ...
                    <tr>
                      ...
                      <td>
                        {{trans('messages.cancel_policy.'.strtolower($result->cancellation))}}
                    ...
                    <tr>
                      <td>
                        @lang('messages.booking.rate') ( @lang('messages.space_detail.per_hour') )
                        <sup class="h6">
                          <i class="icon icon-question tool-amenity2" title="@lang('messages.your_reservations.different_rates')" rel="tooltip"></i>
                        </sup>
                      </td>
                      <td>
                        <span> {{ html_string($result->currency->symbol) }} </span>
                        {{ $result->per_hour }}
                      </td>
                    </tr>
                    @endif
                    ...
                    <tr>
                      <td>
                        @lang('messages.booking.rate') ( @lang('messages.space_detail.per_day') )
                        <sup class="h6">
                          <i class="icon icon-question tool-amenity2" title="@lang('messages.your_reservations.different_rates')" rel="tooltip"></i>
                        </sup>
                      </td>
                    ...
                    <tr>
                      <td>
                        @lang('messages.booking.rate') ( @lang('messages.space_detail.per_week') )
                        <sup class="h6">
                          <i class="icon icon-question tool-amenity2" title="@lang('messages.your_reservations.different_rates')" rel="tooltip"></i>
                        </sup>
                      </td>
                    ...
                    <tr>
                      <td>
                        @lang('messages.booking.rate') ( @lang('messages.space_detail.per_month') )
                        <sup class="h6">
                          <i class="icon icon-question tool-amenity2" title="@lang('messages.your_reservations.different_rates')" rel="tooltip"></i>
                        </sup>
                      </td>
                    ...
                    <tr>
                      <td>
                        ...
                        {{ $result->per_hour }} x {{$result->hours}} 
                        @if($result->hours<=1) 
                      ...
                      <td>
                        <span> {{ html_string($result->currency->symbol) }} </span>
                        {{ $result->base_per_hour * $result->hours}}
                      </td>
                    </tr>
                    @endif
                     @if($result->days>0)
                    ...
                    <tr>
                      <td>
                        ...
                        {{ $result->per_week }} x {{$result->weeks}}
                         @if($result->weeks<=1) 
                    ...
                    <tr>
                      <td>
                        ...
                        {{ $result->per_month }} x {{$result->months}}  
                         @if($result->months<=1) 
                      ...
                      <td>
                        <span> {{ html_string($result->currency->symbol) }} </span>
                        {{ $result->per_month * $result->months}}
                      </td>
                    </tr>
                    @endif
                    ...
                    <tr>
                      <td>
                        @lang('messages.your_reservations.subtotal')
                      </td>
                      <td>
                        <span> {{ html_string($result->currency->symbol) }} </span>
                        {{ $result->subtotal }}
            ...
            <div class="card-body">
              <div class="row">
                <div class="col-12 col-md-4 profile-img user-reserve text-center text-md-left">
                  ...
                  <h4>
                    <a href="{{ route('show_profile',[$result->users->id]) }}" class="verification_user_name theme-link">
