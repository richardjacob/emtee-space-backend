            ...
            <div class="row">
                            ...
                            <li class="listing card-body">
                                <div class="row">
                                    <div class="col-5 col-md-3 col-lg-2 room-image pr-0">
                                        <a href="{{ route('space_details',[$row->id]) }}">
                                    ...
                                    <div class="col-7 col-md-9 col-lg-10 p-0 d-md-flex room-info">
                                        <div class="col-12 col-md-7">
                                            ...
                                            <div class="actions mt-1">
                                                <a class="theme-link" href="{{ route('manage_space', ['space_id' => $row->id,'page' => 'home']) }}">
                                        ...
                                        <div class="list-btn-wrap col-12 col-md-5 mt-2 mt-md-0 d-lg-flex align-items-center">
                                            @if($row->steps_count == 0 && ( $row->status == NULL || $row->status == 'Pending'))
                                                <a class="btn ml-auto" href="{{ route('manage_space', ['space_id' => $row->id,'page' => 'home']) }}">
                                            ...
                                            @else
                                            <div id="availability-dropdown" class="availability-dropdown-wrap d-flex align-items-center justify-content-md-end ml-auto mr-lg-2" data-space-id="div_{{ $row->id }}">
                                                <i class="dot mr-2 dot-{{ ($row->status == 'Listed') ? 'success' : 'danger' }}"></i>
                                                <div class="select">
                                                    <select class="space_status_dropdown" data-space-id="{{ $row->id }}">
                                                        <option value="Listed" {{ ($row->status == 'Listed') ? 'selected' : '' }}>
                                                            {{ trans('messages.your_listing.listed') }}
                                                        </option>
                                                        <option value="Unlisted" {{ ($row->status == 'Unlisted') ? 'selected' : '' }}>
                                                            @lang('messages.your_listing.unlisted')
                                                        </option>
                                                    </select>
                                                </div>
                                            </div>
                                            <a class="btn btn-host step_count mt-2 mt-lg-0 disable_after_click" href="{{ route('duplicate_space',['id' => $row->id]) }}">
                                                @lang('messages.rooms.duplicate')
                                            </a>
                                            @endif
                                        </div>
                                    </div>
                        ...
                        <li class="listing card-body">
                            ...
                            <div class="col-7 col-md-9 col-lg-10 p-0 d-md-flex room-info">
                                <div class="col-12 col-md-7">
                                    ...
                                    <div class="actions mt-1">
                                        <a class="theme-link" href="{{ route('manage_space', ['space_id' => $row->id,'page' => 'home']) }}">{{ trans('messages.your_listing.manage_listing_calendar') }}</a>
                                    </div>
                                </div>
                                <div class="list-btn-wrap col-12 col-md-5 mt-2 mt-md-0 d-lg-flex align-items-center">
                                    @if($row->steps_count == 0 && ($row->status == NULL || $row->admin_status == 'Pending'))
                                    <a class="btn ml-auto" href="{{ route('manage_space', ['space_id' => $row->id,'page' => 'home']) }}">
                                        {{ trans('messages.your_listing.pending') }}
                                    </a>
                                    @elseif($row->steps_count == 0 && $row->admin_status == 'Resubmit')
                                    <a class="btn ml-auto" href="{{ route('manage_space', ['id' => $row->id,'page' => 'home']) }}">
                                        {{ trans('messages.profile.Resubmit') }}
                                    </a>
                                    @elseif($row->steps_count != 0)
                                    <a class="btn ml-auto" href="{{ route('manage_space', ['space_id' => $row->id,'page' => 'home']) }}">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <a href="{{ route('manage_space.new') }}" class="btn list-your-space-btn becomebtn" id="list-your-space">
