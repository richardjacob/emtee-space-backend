			...
			<div class="col-md-8 col-sm-offset-2">
				...
				<div class="box box-info">
					...
					<div class="box-body">
						...
						<div class="form-group">
							<label class="col-sm-3 control-label">
								{{($result->hours==1)?'Total Hour':'Total Hours'}}
							</label>
							<div class="col-sm-6 col-sm-offset-1 form-control-static">
								{{ $result->hours }}
							</div>
						...
						<div class="form-group">
							<label class="col-sm-3 control-label">
								{{($result->days==1)?'Total Day':'Total Days'}}
							</label>
							<div class="col-sm-6 col-sm-offset-1 form-control-static">
								{{ $result->days }}
							</div>
						...
						<div class="form-group">
							<label class="col-sm-3 control-label">
								{{($result->weeks==1)?'Total Week':'Total Weeks'}}
							</label>
							<div class="col-sm-6 col-sm-offset-1 form-control-static">
								{{ $result->weeks }}
							</div>
						...
						<div class="form-group">
							<label class="col-sm-3 control-label">
								{{($result->months==1)?'Total Month':'Total Months'}}
							</label>
							<div class="col-sm-6 col-sm-offset-1 form-control-static">
								{{ $result->months }}
							</div>
						...
						<div class="form-group">
							<label class="col-sm-3 control-label">
								{{ html_string($result->currency->symbol) }}{{$result->per_hour}} x {{$result->hours}} {{($result->hours==1)?'Hour':'Hours'}}
							</label>
							<div class="col-sm-6 col-sm-offset-1 form-control-static">
								{{ html_string($result->currency->symbol) }}{{ $result->per_hour * $result->hours }}
							</div>
						</div>
						@endif
						@if($result->days>0)
						<div class="form-group">
							<label class="col-sm-3 control-label">
								{{ html_string($result->currency->symbol) }}{{$result->per_day}} x {{$result->days}} {{($result->days==1)?'Day':'Days'}}
							</label>
							<div class="col-sm-6 col-sm-offset-1 form-control-static">
								{{ html_string($result->currency->symbol) }}{{ $result->per_day * $result->days }}
							</div>
						</div>
						@endif
						@if($result->weeks>0)
						<div class="form-group">
							<label class="col-sm-3 control-label">
								{{ html_string($result->currency->symbol) }}{{$result->per_week}} x {{$result->weeks}} {{($result->weeks==1)?'Week':'Weeks'}}
							</label>
							<div class="col-sm-6 col-sm-offset-1 form-control-static">
								{{ html_string($result->currency->symbol) }}{{ $result->per_week * $result->weeks}}
							</div>
						</div>
						@endif
						@if($result->months>0)
						<div class="form-group">
							<label class="col-sm-3 control-label">
								{{ html_string($result->currency->symbol) }}{{$result->per_month}} x {{$result->months}} {{($result->months==1)?'Month':'Months'}}
							</label>
							<div class="col-sm-6 col-sm-offset-1 form-control-static">
								{{ html_string($result->currency->symbol) }}{{ $result->per_month * $result->months}}
							</div>
						</div>
						@endif
						...
						<div class="form-group">
							<label class="col-sm-3 control-label">
								{{ ($result->coupon_code == 'Travel_Credit') ? 'Travel Credit' : 'Coupon Amount' }}
							</label>
							<div class="col-sm-6 col-sm-offset-1 form-control-static">
						...
						<div class="form-group">
							<label class="col-sm-3 control-label">
								{{ (@$result->host_payout_preferences->payout_method == 'Stripe') ? 'Host Payout Stripe Account' : 'Host payout email id' }}
							</label>
				...
				@if($result->can_payout_button_show())
					@if((($result->status == 'Accepted' &&  $result->checkin_cross == 0 ) || $result->status == 'Cancelled') && $result->check_host_payout != 'yes' && $result->admin_host_payout != 0)
						@if($result->host_payout_email_id)
							...
							@endif
						@else
							<div class="text-bold text-danger text-center">Yet, host doesn't enter his/her Payout preferences. <a href="{{ url(ADMIN_URL.'/reservation/need_payout_info/'.$result->id.'/host') }}">Send Email to Host</a></div>
						@endif
					@endif
				...
				<div class="box-footer text-center">
					<a class="btn btn-default" href="{{ route('reservations') }}">Back</a>
				</div>
				<!-- /.box-footer -->
			</div>
			<!-- /.box -->
