        ...
        <ul class="wishlists-wrap d-md-flex flex-wrap">
          @foreach($result as $row)
            <li class="col-12 col-md-4">
              <a href="{{ route('wishlists_details',['id' => $row->id]) }}" class="wishlist-bg-img" style="background-image:url('{{@$row->saved_wishlists[0]->photo_name }}');">
                <div class="count-section mt-auto mb-3 col-12">
                  ...
                  <span>
                    @if($row->space_count > 0)
                      {{ $row->space_count }} @lang('messages.new_home.space')
                    @endif
                  </span>
                </div>
              </a>
