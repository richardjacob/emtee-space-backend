            ...
            <ul class="wishlists-wrap d-md-flex flex-wrap mb-5">
              ...
              <li class="col-12 col-md-6">
                <div class="panel-image">
                  <a href="{{ url('space/'.$row->id) }}" class="wishlist-bg-img" target="_blank">
                    <img src="{{ $row->photo_name }}">
                  </a>
                  <span class="price-amount">
                    <sup>
                      {{ html_string($row->activity_price->currency_symbol) }}
                    </sup>
                    {{ $row->activity_price->hourly }}
                  </span>
                  <div class="wishlist-icon">
                    <span class="rich-toggle wish_list_button wishlist-button not_saved" data-hosting_id="{{ $row->id }}" data-img="{{ $row->photo_name }}" data-name="{{ $row->name }}" data-address="{{ $row->space_address->city }}" title="Save to Wish List">
                      ...
                      <label for="wishlist-widget-{{ $row->id }}">
                        ...
                        <i class="icon icon-heart-alt" id="wishlist-widget-icon-{{ $row->id }}" data-toggle="modal" data-target="#wishlist-modal" data-space_id="{{ $row->id }}" data-img="{{ $row->photo_name }}" data-name="{{ $row->name }}" data-address="{{ $row->space_address->city }}" data-host_img="{{ $row->users->profile_picture->src }}"></i>
                      </label>
                    </span>
                  </div>
                ...
                <div class="panel-body">
                  <h4 class="listing-name" title="{{ $row->name }}">
                    <a class="normal-link" href="{{ url('space/'.$row->id) }}" target="_blank">
                      {{ $row->name }}
                    </a>
                  </h4>
                  <h5 class="listing-location">
                    <a href="{{ url('s/?location='.$row->space_address->city) }}">
                      {{ $row->space_address->city }}, {{ $row->space_address->country_name }}
                    </a>
                  </h5>
                </div>
              </li>
