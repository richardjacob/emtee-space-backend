@if(!in_array(Route::CurrentRouteName(),['manage_space','manage_space.new','payment.home','receipt','home_page']))
<button class="d-none d-lg-block footer-toggle">
  <span class="open-footer align-items-center">
    <i class="icon icon-globe mr-2"></i> 
    {{ trans('messages.new_home.terms_privacy_more') }}
  </span>
  <span class="close-footer align-items-center">
    <i class="icon icon-remove mr-2"></i> 
...
@if(in_array(Route::currentRouteName(),['manage_space','manage_space.new','host_conversation']))
...
<script>
  ...
  var first_segment  = "{{ request()->segment(1) }}";
  var max_guest_limit = {{ $max_guest_limit }};

  var space_ajax_url = {
    manage_space        : '{{ url("manage_listing") }}',
    update_space        : '{{ route("update_space") }}',
    photos_list         : '{{ route("photos_list") }}',
    upload_photos       : '{{ route("upload_photos") }}',
    delete_photo        : '{{ route("delete_photo") }}',
    change_photo_order  : '{{ route("change_photo_order") }}',
    photo_description   : '{{ route("photo_description") }}',
    update_price        : '{{ route("update_price") }}',
    calendar_edit       : '{{ route("calendar_edit") }}',
    get_language_list   : '{{ route("get_language_list") }}',
    get_all_trans_desc  : '{{ route("get_all_trans_desc") }}',
    add_description     : '{{ route("add_description") }}',
    delete_description  : '{{ route("delete_description") }}',
    space_calendar      : '{{ route("space_calendar") }}',
    price_calculation   : '{{ route("price_calculation") }}',
    get_min_amount      : '{{ route("get_min_amount") }}',
    reply_message       : '{{ route("reply_message") }}',
    space_activities    : '{{ route("space_activities") }}',
  };

  var common_ajax_url = {
    host_calendar       : '{{ route("host_calendar") }}',
    apply_coupon        : '{{ route("apply_coupon") }}',
    remove_coupon       : '{{ route("remove_coupon") }}',
    login               : '{{ route("user_login") }}',
    save_wishlist       : '{{ route("save_wishlist") }}',
    wishlist_create     : '{{ route("wishlist_create") }}',
  }
...
@if (Route::currentRouteName() == 'new_space')
{!! Html::script('js/new_space.js?v='.$version) !!}
{!! Html::script('js/home_two.js?v='.$version) !!}
@endif

@if (Route::currentRouteName() == 'manage_space' || Route::currentRouteName() == 'manage_space.new')
...
{!! Html::script('js/home_two.js?v='.$version) !!}
@endif
@if (Route::currentRouteName() == 'current_bookings')   
{!! Html::script('js/home_two.js?v='.$version) !!} 
@endif
@if (Route::currentRouteName() == 'previous_bookings')   
{!! Html::script('js/home_two.js?v='.$version) !!} 
@endif
...
{!! Html::script('js/space.js?v='.$version) !!}
{!! Html::script('js/home_two.js?v='.$version) !!}
@endif
...
<div class="takephoto-popup modal fade" role="dialog" id="takephoto-popup">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5>Take Your Photo</h5>
        <button type="button" class="close" data-dismiss="modal" onclick="clear_takepic()"></button>
      </div>
        <div class="modal-body">
          <div class="d-flex my-3 align-items-center" id='my_camera'>
          </div>
          <input class="btn btn-primary" value="Take Photo" onClick="take_snapshot()">
          <div class="d-flex my-3 align-items-center" id='takephoto_result'>
          </div>
           <form name="takephoto_upload_form" method="post" id="takephoto_upload_form" enctype="multipart/form-data" action="{{ url('users/image_takephoto') }}" accept-charset="UTF-8">
            {!! Form::token() !!}
            <input type="hidden" name="user_takephoto_pic" id="user_takephoto_pic">
            <input class="btn btn-success" value="Save Photo" id='takephoto_button' style="display: none;">
          </form>
        </div>
      </div>
  </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <ul class="social-log">
          ...
          <li>
            <a href="{{ getAppleLoginUrl() }}" id="apple_signup_login" class="apple-btn btn d-flex align-items-center justify-content-center">
              <i class="fa fa-apple push-half--right"></i>
              <span class="d-inline-block ml-3">
          ...
          <div class="my-3">
            ...
            <div class="log-form">
              ...
              <div class="signup-form-fields">
                ...
                {!! Form::submit( trans('messages.header.signup'), ['class' => 'my-3 d-flex justify-content-center btn btn-primary disable_after_click' , 'id' => 'user-signup-btn', 'ng-disabled' => '!agree_toc'])  !!}
                {!! Form::close() !!}
