...
$(document).on('click','.add_lang_amenity',function(){  
    count++;
 ...
 */
$(document).on('focusin','.admin_space',function(){
    // store the previous room status
    $(this).data('prev_value', $(this).find(":selected").val());
}).on('change','.admin_space',function(){
    $space_id =$(this).attr('id');
    ...
    if($value !='')
    {
        if($value ==  'Resubmit'){
            $('#resubmit_msg').val('');
            $('#resubmit_space_id').val($space_id);
            ...
            $('#resubmit_listing').css('display','block');
             ...
             window.location = APP_URL+'/'+ADMIN_URL+'/update_space_status/'+$space_id+'/'+$type+'/'+$value;
        }
...
$(document).on('focusin','#resubmit_cancel',function(){
    $space_id = $('#resubmit_space_id').val();
    $prev_status = $('#resubmit_prev_val').val();
    $("#"+$space_id).val($prev_status);
});
...
$(document).on('click','.resubmit',function(){
    ...
    var space_id = $('#resubmit_space_id').val();
    if(msg.trim() != ''){
        $('.resubmit_err_msg').addClass('hide'); 
        $.ajax({
            type: "POST",
            url: APP_URL+'/'+ADMIN_URL+'/resubmit_listing',
            data: {space_id:space_id,msg:msg},
            dataType:"html",
            async:false,
            success: function(msg) { 
                window.location = APP_URL+'/'+ADMIN_URL+'/spaces';
            }
