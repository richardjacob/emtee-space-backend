...
$(document).ready(function() {
    ...
    $('.footer-toggle').click(function() {
        $('.cls_showftr').toggleClass('footer-shown');
    ...
    $('.more_menu').click(function() {
        $('.frthide').css('height' , '0').css('opacity' , '0');
        $('.ftrshow').css('height' , 'auto').css('opacity' , '1');
    });
    $('.hide_menu').click(function() {
        $('.frthide').css('height' , 'auto').css('opacity' , '1');
        $('.ftrshow').css('height' , '0').css('opacity' , '0');
    });
...
function countChar(val,type='empty') {console.log(type);
...
$("#address_line").keypress(function(e) {
        if (e.keyCode === 13) {
            e.preventDefault();
        }
});
...
function headerAutocomplete() {
    if (document.getElementById('header-search-form')) {
        ...
        google.maps.event.addListener(header_autocomplete, 'place_changed', function() {
            ...
            var last_part = current_url.substr(current_url.lastIndexOf('/'));
            if(last_part == '/') {
                $('#activity_select').selectpicker('toggle');
            }
            else {
...
$('#search-form').submit(function(event) {
    if ($('#location').val() == '') {
        $('.search_location_error').removeClass('d-none');
...
app.controller('payment', ['$scope', '$http', function($scope, $http) {
    ...
    $('#apply-coupon').click(function() {
        ...
        var coupon_url = common_ajax_url.apply_coupon;
...
app.controller('footer', ['$scope', '$http', '$rootScope', function($scope, $http, $rootScope) {
    ...
    $('.space_status_dropdown').change(function() {
        var url     = space_ajax_url.update_space;
        var status  = $(this).val();
        var space_id = $(this).data('space-id');
        var data    = {'space_id' : space_id,'user_id' : USER_ID, step : 'basics', status : status };

        $http.post(url, data).then(function(response) {
            var selector = '[data-space-id="div_' + space_id + '"] > i';
            if (status == 'Listed') {
                $(selector).removeClass('dot-danger').addClass('dot-success');
            }
            else {
                $(selector).addClass('dot-danger').removeClass('dot-success');
            }
        });
    });
...
$(document).on('click', '#takephoto_button', function() {
     $('#takephoto-popup').modal('hide');
     $('#takephoto_upload_form').submit();
});
...
$(window).on('load', function() {
    $(document).ready(function () {
        $(window).on("scroll touchmove",function() {
            ...
            if ($(window).width() > 767) {
                $('.home-info').css('z-index', '99');
                $('.bootstrap-select div.dropdown-menu').removeClass('show');
            }
        });
        $('.selectpicker').on('shown.bs.select', function (e, clickedIndex, isSelected, previousValue) {
            $('.home-info').css('z-index', '9999');
        });
...
$(document).ready(function() {
    var cls_hmslder;
    cls_hmslder = $('.cls_hmslder.owl-carousel').owlCarousel({
       lazyLoad:true,
        margin:0,
        nav:false,
        dots:false,
    ...
    cls_hmslder.on('changed.owl.carousel', function(e) {
        cls_hmslder.trigger('stop.owl.autoplay');
        cls_hmslder.trigger('play.owl.autoplay');
    });
});
...
function header_fixed() {
    ...
    $('.inner_page main').css({ "margin-top": 0 + "px" });
    $('.space-detail-page.inner_page main').css({ "margin-top": header_height + "px" });
}
function menu_fixed() {
    var header_height = $('header').outerHeight();
...
$(document).ready(function() {
    ...
    $('.cls_headerdown').on('show.bs.dropdown', function () {
        $('.cls_header').css('z-index','9999');
    })
    $('.cls_headerdown').on('hide.bs.dropdown', function () {
        $('.cls_header').css('z-index','999');
    })
...
$(document).ready(function() {
   ...
   $(window).resize(function() {
    ...
    $('.login_popup_open').click(function(event) {
...
app.controller('home_owl', function($scope, $http) {
    ...
    $scope.ajax_home = function() {
        ...
        .then(function(response) {
            ...
            setTimeout( () => { 
                ...
                $scope.update_slider('.featured_category-slider');
                $('.whole-slider-wrap').removeClass("dot-loading");
                // Load our Community and popular venues
                // $('.popular_venue').addClass('dot-loading');
    ...
    //     .then(function(response) {
    //         $scope.popular_activities = response.data.popular_activities;
    //         $scope.our_community    = response.data.our_community_banners;
    //         setTimeout(() => {
    //             $scope.update_slider('.community-content','community');
    //             $('.popular_venue').removeClass('dot-loading')
    //         },100);
    //     });
    // };
...
$(window).on('scroll',function(){
    var header_height = $('.cls_header').outerHeight();
    var window_scroll = $(window).scrollTop();
    if (window_scroll > header_height && $(window).width() > 992) {
      $('.cls_header').addClass('reguler-header');
    }
    else {
      $('.cls_header').removeClass('reguler-header');
   }
});
...
$(window).on('scroll',function(){
  ...
  footheader.toggleClass('hidden', scrollTop > prev);
  prev = scrollTop;
});


$('.cls_count').each(function () {
...
$(function() {
  $(".profile_settings1").on("click", function(e) {
    $(".cls_mobileprofile").addClass("d-block");
    $('body').css('overflow', 'hidden');
    e.stopPropagation()
  });
  // $(document).on("click", function(e) {
  //   if ($(e.target).is(".cls_mobileprofile") === false) {
  //     $(".cls_mobileprofile").removeClass("d-block");
...
$('.Publish-btn').click(function() {
    $('.pending-list').addClass('d-block');
    $('.publish-list').addClass('d-none');
...
// $('.pending-list').click(function() {
//     $('.publish-list').addClass('show');
