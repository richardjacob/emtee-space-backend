...
{    
    ...
    public function query(Referrals $model)
    {
        $referrals = $model::join('users', function($join) {
                            ...
                            })->selectRaw('referrals.user_id, CONCAT(users.first_name, " ", users.last_name) as full_name, referrals.friend_id, (select count(*) from referrals as r1 where user_id = referrals.user_id) as signup_count, (select count(*) from reservation as re1 join referrals as jr on jr.friend_id = re1.user_id where jr.user_id = referrals.user_id) as booking_count, (select count(*) from space as rm1 join referrals as jr on jr.friend_id = rm1.user_id where jr.user_id = referrals.user_id) as listing_count')->groupBy('referrals.user_id');
