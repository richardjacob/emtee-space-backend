...
class ReservationsController extends Controller
{
    ...
    {
        $reservation_id = Reservation::findOrFail($request->id);

        if(!$_POST) {
            $data['result'] = $result = Reservation::with('users','reservation_times','activities','sub_activities','host_payout_preferences','currency','host_penalty','hostPayouts')->find($request->id);

            if($data['result']['cancelled_by'] == "Guest")
            {
                $data['cancel_message']=DB::table('messages')->where('reservation_id','=',$request->id)->where('message_type','=','10')->value('message');
            }
            else
            {
                $data['cancel_message']=DB::table('messages')->where('reservation_id','=',$request->id)->where('message_type','=','11')->value('message');
            }
    ...
    public function payout(Request $request, EmailController $email_controller)
    {
        ...
        if($request->user_type == 'host')
        {
            ...
            $amount = currency_convert($reservation_details->currency_code, $payout_currency, $reservation_details->host_payout);
            ...
            $payouts->space_id              = $reservation_details->space_id;
        ...
        if($request->user_type == 'guest') {
            ...
            $amount             = currency_convert($reservation_details->currency_code, $payout_currency, $reservation_details->guest_payout);
