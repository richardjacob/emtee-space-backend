...
class AmenitiesController extends Controller
{
    ...
    public function add(Request $request)
    {
    	...
    	else if($request->submit)
        {
        	...
        	if(@$amenities_name->count() != 0) {
                flash_message('error', 'This Name already exists'); // Call flash message function
    ...
    public function update(Request $request)
    {
        if(!$_POST)
        {
	        ...
	        $amenity_del = AmenitiesLang::select('id')->where('amenities_id',$request->id)->get();
	        foreach($amenity_del as $values) { 
        	...
        	$amenities_count = Amenities::where('id','!=',@$request->id)->where('name','=',@$request->name[0])->count();            
	...
	*/
	...
	{    
		...
		if(!is_null($amenities)) {
			...
			flash_message('warning', 'Already Deleted'); 
		}
		return redirect(ADMIN_URL.'/amenities');
	}
}