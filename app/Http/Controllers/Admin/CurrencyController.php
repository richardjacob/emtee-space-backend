...
/**
    ...
    public function canDestroy($id, $code)
    {
        ...
        $is_space_currency = SpacePrice::where('currency_code', $code)->count();
        $is_space_act_currency = ActivityPrice::where('currency_code', $code)->count();
        ...
        else if($is_space_currency > 0)
        {
            $return = ['status' => 0, 'message' => 'Sorry, Space have this Currency. So, Delete that Space or Change that Space Currency.'];   
        }
        else if($is_space_act_currency > 0)
        {
            $return = ['status' => 0, 'message' => 'Sorry, Space have this Currency. So, Delete that Space or Change that Space Currency.'];   
        }
