 ...
 */
	...
	public function review_detail(Request $request) {
		...
		if ($validator->fails()) {
			...
			//get review details
			$reviews = Reviews::where('space_id', $request->room_id)
		...
		if ($validator->fails()) {
			...
			if ($data->status == 'Pending') {
				...
				return response()->json([
					...
					'room_status' => ($data->room_status=='steps_remaining')?(string) $de_result['data'][$i]['steps_count'].' '.trans('messages.your_listing.steps_to_list'):trans('messages.your_listing.'.$data->room_status),
			...
			if ($data->status == 'Listed') {
				...
				return response()->json([
					...
					'room_status' => ($room->room_status=='steps_remaining')?(string) $de_result['data'][$i]['steps_count'].' '.trans('messages.your_listing.steps_to_list'):trans('messages.your_listing.'.$room->room_status),
			...
			if ($de_result['total'] == 0 || empty($de_result['data'])) {
				...
				for ($i = 0; $i < count($de_result['data']); $i++) {
					...
					$data[] = array(
						...
						'room_status' => ($room_status=='steps_remaining')?(string) $de_result['data'][$i]['steps_count'].' '.trans('messages.your_listing.steps_to_list'):trans('messages.your_listing.'.$room_status),
