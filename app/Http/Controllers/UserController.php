...
class UserController extends Controller
{
    ...
    public function facebookAuthenticate(EmailController $email_controller, Request $request)
    {
        ...
        if($user->count() > 0 ) {
            ...
            $user = User::user_facebook_authenticate($email, $facebook_id)->withTrashed();
            ...
            if(session('referral')) {
                ...
                $referral_check = Referrals::whereUserId(session('referral'))->sum('creditable_amount');

                $referral = new Referrals;

                $referral->user_id                = session('referral');
    ...
    public function create(Request $request, EmailController $email_controller)
    {
        ...
        if(session('referral')) {
            $referral_settings = ReferralSettings::first();
            $referral_check = Referrals::whereUserId(session('referral'))->get()->sum('creditable_amount');
    ...
    public function googleAuthenticate(EmailController $email_controller, Request $request)
    {
        ...
        if($users->status != 'Inactive') {
            ...
            if(Auth::guard()->loginUsingId($user_id)) {
                if(session('ajax_redirect_url')) {
                    return redirect()->intended(session('ajax_redirect_url')); // Redirect to ajax url 
                }
                return redirect()->intended('dashboard'); // Redirect to dashboard page
            }
            else {
    ...
    public function dashboard()
    {
        ...
        $data['all_message'] = $all_message = Messages::whereIn('id', function($query) use ($user_id) {
            ...
            ->with(['reservation' => function($query) {
                $query->with(['currency', 'space' => function($query) {
        ...
        $host_future_payout = payouts::join('reservation', function($join) {
            ...
            ->select('payouts.amount','reservation.hours','reservation.space_id','payouts.currency_code')
        ...
        $future_payout = 0;
        $future_nights = 0;
        $total_rooms = count($host_future_payout);
        ...
        $host_completed_payout = payouts::join('reservation', function($join) {
            ...
            ->select('payouts.amount','reservation.hours','payouts.currency_code')
            ->where('payouts.user_id','=',auth()->id())
            ->where('payouts.user_type','host')
        ...
        $completed_payout=0;
        $completed_nights=0;
        $total_rooms1 = count($host_completed_payout);
        ...
        $data['pending']  = Messages::whereIn('id', function($query) use($user_id) {
            ...
            ->with('user_details.profile_picture','reservation.currency','space_address','reservation')
    ...
    public function edit()
    {
        ...
        $dob = ' - - ';
        if (auth()->user()->dob) {
            $dob=date('Y-m-d',custom_strtotime(auth()->user()->dob));
        }
    ...
    public function confirm_email(Request $request)
    {
        ...
        $user = auth()->user();
        $user->load('users_verification');
        $user_verification = $user->users_verification;

        if($user_verification->email == 'yes') {
            flash_message('danger', __('messages.login.email_already_confirmed'));
            return redirect('dashboard');
        }
        ...
        if($password_resets->count() && auth()->user()->email == $password_resets->first()->email) {
            ...
            if($hours >= 1) {
                ...
                flash_message('danger', __('messages.login.token_expired'));
                return redirect('dashboard');
            }
            ...
            flash_message('success', __('messages.profile.email_confirmed'));
        }
        else {
            flash_message('danger', __('messages.login.invalid_token'));
        }
        return redirect('dashboard');
    }
    ...
    {
        $user_id        = $request->id;
        $data['result'] = User::findOrFail($user_id);

        $data['reviews_from_guests'] = Reviews::where(['user_to' => $user_id, 'review_by' => 'guest']);
        $data['reviews_from_hosts'] = Reviews::where(['user_to' => $user_id, 'review_by' => 'host']);

        $data['reviews_count'] = $data['reviews_from_guests']->count() + $data['reviews_from_hosts']->count();

        $wish_list = Wishlists::with('saved_wishlists.space','profile_picture')->where('user_id', $user_id);

        if(auth()->id() != $user_id) {
            $wish_list = $wish_list->where('privacy','0');
        }

        $data['wishlists'] = $wish_list->orderBy('id', 'desc')->get();
        $data['title']     = $data['result']->first_name."'s Profile ";

        $data['space'] = Space::with('activity_price.currency','users')->whereHas('users', function ($query) {
    ...
    public function payout_preferences(Request $request, EmailController $email_controller)
    {
        if(!$request->address1)
        { 
            $data['payouts'] = PayoutPreferences::where('user_id', auth()->id())->orderBy('id','desc')->get();
        ...
        $payout_check = PayoutPreferences::where('user_id', auth()->id())->where('default','yes')->get();
        ...
        return redirect('users/payout_preferences/'.auth()->id());
    }
    ...
    public function update_payout_preferences(Request $request, EmailController $email_controller, $id)
    {
        ...
        $nice_names = array(
            ...
            'additional_document'  => trans('messages.account.additional_document'),
    ...
    public function stripe_payout_preferences(Request $request)
    {
        ...
        $session_payout_data = session('payout_preferences_data');
    ...
    public function payout_default(Request $request, EmailController $email_controller)
    {
        ...
        if($payout->default == 'yes') {
            ...
            $payout_all = PayoutPreferences::where('user_id',auth()->id())->update(['default'=>'no']);
    ...
    {
        $list = Space::where('user_id', auth()->id())->whereIn('status',['Listed','Unlisted'])->get();
        $data['lists']=$list->pluck('name','id');
        $data['payout_methods'] = PayoutPreferences::where('user_id', auth()->id())->pluck('paypal_email','id');
   ...
   public function transaction_history_csv(Request $request)
    {
        ...
        {
            unset($transaction[$i]['id']); unset($transaction[$i]['reservation_id']);unset($transaction[$i]['space_id']);unset($transaction[$i]['user_id']); unset($transaction[$i]['status']); unset($transaction[$i]['penalty_id']);
    ...
    {
        $data['reviews_about_you'] = Reviews::where('user_to', auth()->id())->orderBy('id', 'desc')->get();
        $data['reviews_by_you'] = Reviews::where('user_from', auth()->id())->orderBy('id', 'desc')->get();

        $data['reviews_to_write'] = Reservation::with('reviews','reservation_times')
            ->where('status', 'Accepted')
            ->whereHas('reservation_times', function($query) {
               $query->whereRaw('DATEDIFF(now(),end_date) <= 14')->whereRaw('DATEDIFF(now(),end_date) >= 1');
            })
        ...
        $data['expired_reviews'] = Reservation::with('reviews','reservation_times')
            ->where('status', 'Accepted')
            ->whereHas('reservation_times', function($query) {
               $query->whereRaw('DATEDIFF(now(),end_date) > 14');
            })
            ->where(function($query) {
                return $query->where('user_id', auth()->id())->orWhere('host_id', auth()->id());
            })
        ...
        for($i=0; $i<$data['reviews_to_write']->count(); $i++) {
            if($data['reviews_to_write'][$i]->review_days > 0 && $data['reviews_to_write'][$i]->reviews->count() < 2) {
                ...
                for($j=0; $j<$data['reviews_to_write'][$i]->reviews->count(); $j++) {
                    if(@$data['reviews_to_write'][$i]->reviews[$j]->user_from != auth()->id())
                        $data['reviews_to_write_count'] += 1;
                }
            }
        }
        ...
        for($i=0; $i<$data['expired_reviews']->count(); $i++) {
            if($data['expired_reviews'][$i]->review_days <= 0 && $data['expired_reviews'][$i]->reviews->count() < 2) {
                ...
                for($j=0; $j<$data['expired_reviews'][$i]->reviews->count(); $j++) {
                    if(@$data['expired_reviews'][$i]->reviews[$j]->user_from != auth()->id())
                        $data['expired_reviews_count'] += 1;
                }
            }
        }
   ...
   public function reviews_edit(Request $request, EmailController $email_controller)
    {
        $data['result'] = $reservation_details = Reservation::with('space.users.profile_picture','space.space_address')->find($request->id);

        if(!empty($reservation_details)) {
            if(auth()->id() == $reservation_details->user_id) {
            ...
            if(!$request->data) {
                ...
                else if($reservation_details->host_id == auth()->id()) {
                ...
                $reviews->space_id = $reservation_details->space_id;
     ...
     */
      public function image_takephoto(Request $request){
       
        $image = $request->user_takephoto_pic; 
        $user_id = auth()->id();
        
            if(UPLOAD_DRIVER=='cloudinary') {
                try  {
                    $last_src=DB::table('profile_picture')->where('user_id',$user_id)->first()->src;
                ...
                $filename = str_random(5).date('HmdY').'.jpeg';

                $path = dirname($_SERVER['SCRIPT_FILENAME']).'/images/users/'.$user_id.'/';            
                if(!file_exists($path)) {
                    mkdir(dirname($_SERVER['SCRIPT_FILENAME']).'/images/users/'.$user_id.'/', 0777, true);
                }
                $success = file_put_contents($path.$filename, $data);

                $compress_success = $this->helper->compress_image('images/users/'.$user_id.'/'.$filename, 'images/users/'.$user_id.'/'.$filename, 80);
                $compress_success = $this->helper->compress_image('images/users/'.$user_id.'/'.$filename, 'images/users/'.$user_id.'/'.$filename, 80,510,510);
            }
            ...
            if($remove_file){
                ...
                $this->helper->remove_image_file($remove_file,'images/users/'.$user_id,$compress_images);
            }
            flash_message('success', trans('messages.profile.picture_uploaded'));
    ...
    public function facebookConnect(Request $request)
    {
        ...
        $verification = UsersVerification::find(auth()->id());
    ...
    {
        $id_documents = UsersVerificationDocuments::whereType('id_document')->where('user_id', auth()->id())->get();
        $user = User::find(auth()->id());
    ...
    {
        $doc = UsersVerificationDocuments::where('id',$request->image_id)->where('user_id', auth()->id())->first();
        $user = User::find(auth()->id());
        ...
        if ($doc != NULL) {
            ...
            /*delete file from server*/
            if($remove_file){
                $this->helper->remove_image_file($remove_file,'images/users/'.auth()->id().'/documents');
            }
            /*delete file from server*/
        }
        else {
        ...
        $this->update_verification_status(auth()->id());
        $user = User::find(auth()->id());
    ...
    public function upload_verification_documents(Request $request) {
        ...
        foreach ($request->file as $key => $value) {
            ...
            if (in_array($extension, ['png', 'jpg', 'jpeg', 'PNG', 'JPG', 'JPEG', 'pdf','PDF'])) {
                ...
                $error = array('error_title' => ' Photo Error', 'error_description' => __('validation.mimes',['attribute' => 'File','values'=>'Jpg, Jpeg, Png, Pdf']));
            }
        }
        $this->update_verification_status(auth()->id());
